...
class VoiceTrackerBot {
	 ...
	 */
	constructor({ token, guildId, voiceChannelId, textChannelId }) {
		this.token = token;
	 ...
	 * Resolution and framerate mengikuti asli video (tidak di-rescale),
	 * menggunakan customInputOptions untuk native FFmpeg loop (-stream_loop -1)
	 * dan hardware acceleration, serta customFfmpegFlags untuk thread tuning.
	...
	async startLoopStream(videoPath) {
		while (!this.isShuttingDown) {
			...
			const { command, output } = prepareStream(videoPath, {
				...
				customInputOptions: ["-stream_loop", "-1", "-hwaccel", "auto"],
